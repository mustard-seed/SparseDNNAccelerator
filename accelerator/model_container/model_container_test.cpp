...
TEST(MODEL_CONTAINER_TEST, LoadTrace)
{
    ...
    std::string parameterFileName = "testTrace_parameters.npz";

    YAML::Node traceNodes = YAML::LoadFile(traceFileName);
    cnpy::npz_t parameterNodes = cnpy::npz_load(parameterFileName);
    ...
    EXPECT_EQ((dynamic_pointer_cast<AveragePoolLayer>(vecLayers.at(13)))->getOutputChannel(), 64);
    EXPECT_FLOAT_EQ((dynamic_pointer_cast<AveragePoolLayer>(vecLayers.at(13)))->getDivisor(), 64.0f);
}
