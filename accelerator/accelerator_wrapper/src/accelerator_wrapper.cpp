...
//            #endif
//           //kernelIAMover.setArg(2, (cl_uint) vecLayerInfo.at(i).numIAMoverInstruction);
//           //kernelIAMover.setArg(3, (cl_uint) vecLayerInfo.at(i).offsetIAMoverInstruction);
    ...
    void AcceleratorWrapper::dumpRuntimeToCSV(std::string csvFilePath)
    {
        ...
        if (numRunExecuted > 0)
        {
            ...
            for (unsigned int i=0; i<vecLayerExecutionTime.size(); i++)
            {
                ...
                int kernelSize = layerInfo.kernelSize;
                auto layerTime = vecLayerExecutionTime.at(i);
                std::string name = layerInfo.layerName;
                if (name.length() > maxName)
                {
