...
#define ACTIVATION_BURST_SIZE_GEQ_PE_SIZE 1
...
#endif
#if (ACTIVATION_BURST_SIZE_GEQ_PE_SIZE == 1) //Activation dram size in word is greater than or equal to activation PE block size
...
#elif (ACTIVATION_BURST_SIZE_GEQ_PE_SIZE == 0) 
	#if (PE_ACTIVATION_BLOCK_SIZE_IN_WORD < ACTIVATION_WIDE_SIZE)
		#error ACTIVATION_BURST_SIZE_GEQ_PE_SIZE is 0, but PE_ACTIVATION_BLOCK_SIZE_IN_WORD < PE_ACTIVATION_BLOCK_SIZE_IN_WORD
	#else
		#define ACTIVATION_BURST_SIZE_BYTE (PE_ACTIVATION_BLOCK_SIZE_IN_WORD / ACTIVATION_WIDE_SIZE)
		#define ACTIVATION_BURST_SIZE_BYTE_OFFSET (PE_ACTIVATION_BLOCK_SIZE_IN_WORD_OFFSET - ACTIVATION_WIDE_SIZE_OFFSET)
		#define ACTIVATION_WIDE_SIZE_BYTE_MASK ((1 << ACTIVATION_BURST_SIZE_BYTE_OFFSET) - 1)
	#endif
#else
	#error ACTIVATION_BURST_SIZE_GREATER_THAN_PE_SIZE should be either 0 (false) or 1 (true)
#endif
#if (ACTIVATION_BURST_SIZE_BYTE < PE_ROWS_PER_GROUP)
	#error ACTIVATION_BURST_SIZE_BYTE should be greater than or equal to PE_ROWS_PER_GROUP
#else
...
#define WEIGHT_BURST_SIZE_GEQ_PE_SIZE 0
#define WEIGHT_WIDE_SIZE 2
//============Derived Marcos for weights===
#if (WEIGHT_WIDE_SIZE == 1)
	#define WEIGHT_WIDE_SIZE_OFFSET 0x0 //Numnber of bit-shift that correspond to the conversion factor
	#define WEIGHT_WIDE_SIZE_REMAINDER_MASK 0x0 //Mask bit required for calculating the remainder of the conversion factor
	...
	#define 	WEIGHT_BURST_SIZE_VALUE_BYTE (WEIGHT_WIDE_SIZE * PE_WEIGHT_BLOCK_SIZE_IN_WORD)
	#define WEIGHT_BURST_SIZE_VALUE_BYTE_OFFSET (PE_WEIGHT_BLOCK_SIZE_IN_WORD_OFFSET + WEIGHT_WIDE_SIZE_OFFSET)
	#if defined(SPW_SYSTEM)
		#define WEIGHT_BURST_SIZE_INDEX_BYTE (WEIGHT_WIDE_SIZE*INDEX_CHAR_ARRAY_SIZE)
	#endif
#elif (WEIGHT_BURST_SIZE_GEQ_PE_SIZE == 0)
	#if (PE_WEIGHT_BLOCK_SIZE_IN_WORD < WEIGHT_WIDE_SIZE)
		#error WEIGHT_BURST_SIZE_GEQ_PE_SIZE is 0 but PE_WEIGHT_BLOCK_SIZE_IN_WORD < WEIGHT_WIDE_SIZE
	#endif
	#if defined(SPW_SYSTEM)
		#if (INDEX_CHAR_ARRAY_SIZE < WEIGHT_WIDE_SIZE)
			#error WEIGHT_BURST_SIZE_GEQ_PE_SIZE is 0 but INDEX_CHAR_ARRAY_SIZE < WEIGHT_WIDE_SIZE
		#endif
	#endif
	#define WEIGHT_BURST_SIZE_VALUE_BYTE (PE_WEIGHT_BLOCK_SIZE_IN_WORD / WEIGHT_WIDE_SIZE)
	#define WEIGHT_BURST_SIZE_VALUE_BYTE_OFFSET (PE_WEIGHT_BLOCK_SIZE_IN_WORD_OFFSET - WEIGHT_WIDE_SIZE_OFFSET)
	#if defined(SPW_SYSTEM)
		#define WEIGHT_BURST_SIZE_INDEX_BYTE (INDEX_CHAR_ARRAY_SIZE / WEIGHT_WIDE_SIZE)
		#define WEIGHT_BURST_SIZE_INDEX_BYTE_OFFSET (INDEX_CHAR_ARRAY_SIZE_OFFSET - WEIGHT_WIDE_SIZE_OFFSET)
	#endif
	#else
	#error WEIGHT_BURST_SIZE_GEQ_PE_SIZE must be either 0 (false) or 1 (true)
#endif
